...
int main (int argc, char *argv[]) {
	...
	int socketMemoria,socketFS,socketListen,socketCPU=0,socketConsola=0;
	int conectados, fd;
				...
				if (FD_ISSET(i, &read_fds)) {
					...
					if (i == socketListen) {
						...
						if(fdSocket[i].activo == 0){
							...
							if(recibirHandshake(i, KERNEL_HSK, &codigoHandshake, logger)){
								//ERROR
								return EXIT_FAILURE;
							}
							if(!socketConsola && codigoHandshake == CONSOLA_HSK){
								...
								close(i); //No se reconoce el codigo del Handshake.
							}
